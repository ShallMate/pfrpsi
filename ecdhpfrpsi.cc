...
std::vector<int32_t> EcdhPsiRecv(
  ...
  std::copy_n(std::istream_iterator<int>(file), cuckoosize,
              std::back_inserter(receivershares));
  file.close();  // 关闭文件

  size_t size_x = x.size() * 3;
  std::vector<yc::EcPoint> x_points(size_x);
  alice.MaskUint128s_Recv(absl::MakeSpan(x), absl::MakeSpan(receivershares),
                          absl::MakeSpan(x_points), cuckoosize);

  // Send H(id)^a
  uint64_t max_point_length =
      alice.ec_->GetSerializeLength();  // 获取点的最大序列化长度
...
void EcdhPsiSend(const std::shared_ptr<yacl::link::Context>& ctx,
  ...
  bob.MaskUint128s_Sender(absl::MakeSpan(cuckooHash.bins_),
                          absl::MakeSpan(sendershares),
                          absl::MakeSpan(y_points));
  uint64_t max_point_length =
...
void EcdhPsi::MaskUint128s_Sender(absl::Span<uint128_t> in,
  ...
  yacl::parallel_for(0, in.size(), [&](size_t begin, size_t end) {
    for (size_t idx = begin; idx < end; ++idx) {
      ...
      out[idx] =
          ec_->HashToCurve(yc::HashToCurveStrategy::Autonomous, ssstring);
      ec_->MulInplace(&out[idx], sk_);
    }
  });
}
...
void EcdhPsi::BuffertoStrings(absl::Span<std::uint8_t> in,
